...
export default function MissingConditionEditModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Use the Next.js API route instead of directly calling the external service
      const response = await fetch(`/api/missing-conditions/${condition.id}`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        throw new Error(
          errorData.error || 'Failed to update missing condition'
        );
      }
    } catch (error) {
      ...
      alert(
        error instanceof Error
          ? error.message
          : 'Failed to update missing condition'
      );
    } finally {
